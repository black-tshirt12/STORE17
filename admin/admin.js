...
(function () {
  ...
  const i18n = {
    ar: {
      ...
      syncWhatsappNumber: 'رقم واتساب للطلبات',
      syncWhatsappNumberPlaceholder: '212600000000',
      syncWhatsappNumberHint: 'رقم مع رمز الدولة فقط (بدون https:// أو wa.me). عند الضغط على «إرسال الطلب» يُفتح واتساب مع رسالة جاهزة.'
    },
    en: {
      ...
      syncWhatsappNumberPlaceholder: '212600000000',
      syncWhatsappNumberHint: 'Country code + number only (no https:// or wa.me). When the customer clicks Send Order they are redirected to WhatsApp with the order message.'
    }
  };
  ...
  function renderSyncSection() {
    ...
    container.innerHTML = `
      ...
      <div class="form-group">
        ...
        <p class="form-hint">${t('syncWhatsappNumberHint')}</p>
      </div>
      <div class="form-group">
        <label for="sync-orders-api-url" data-i18n="syncOrdersApiUrl">Orders API URL (admin/dashboard)</label>
        <input type="url" id="sync-orders-api-url" value="${escapeHtml(ordersApiUrl || '')}" placeholder="${t('syncOrdersApiPlaceholder')}" />
        <p class="form-hint">${getLang() === 'ar' ? 'اختياري: لحفظ الطلبات في لوحة التحكم (GitHub). إذا استخدمت واتساب أعلاه، يمكن ترك هذا فارغاً.' : 'Optional: For orders in admin dashboard (GitHub). Leave empty if using WhatsApp orders only.'}</p>
      </div>
      <div class="form-group">
    ...
    document.getElementById('btn-save-sync-config')?.addEventListener('click', function () {
      ...
      const whatsappNumberVal = (document.getElementById('sync-whatsapp-number')?.value || '').trim();
